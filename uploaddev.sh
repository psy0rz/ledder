...
find $STUFF| entr -r -s "rsync -avx . root@$IP:ledder && ssh root@$IP 'pkill node; cd /root/ledder; NODE_ENV=development node src/js/server/server.js '"
