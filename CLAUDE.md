...
- **`Scheduler`** drives everything frame-based: `scheduler.interval(frames, callback)` runs the callback every N display frames; `intervalControlled(controlValue, cb)` ties the rate to a user control. Returning `false` from a callback cancels the interval. `timeToFrames()`/`delayTime()` convert seconds using the current framerate.
- **`RenderSettings`** holds how the animation is rendered instead of what: framerate and subpixel filtering. These belong to the *user*, not to the animation: the renderer owns one instance, and `AnimationManager` creates its controls ("Render settings") in the root `ControlGroup` of the selected animation, so they are always visible and saved in its preset. Animations have no API to change them — every layer and sub-animation shares one framerate and one filtering mode, so letting animations set them would mean the last one to run wins.
