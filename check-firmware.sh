...
if ping -c 1 -W 2 192.168.1.45 >/dev/null 2>&1; then
    ...
    if command -v nc >/dev/null 2>&1; then
        if echo "test" | timeout 2 nc -u -w1 192.168.1.45 5568 2>/dev/null; then
            ...
            echo "   node test-5a75b.js 192.168.1.45 128 64"
            exit 0
        else
    ...
    echo "   Try the LED test to verify: node test-5a75b.js 192.168.1.45 128 64"
...
if ping -c 1 -W 2 192.168.1.45 >/dev/null 2>&1; then
    ...
    echo "1. node test-5a75b.js 192.168.1.45 128 64"
    ...
    echo "5. Test: node test-5a75b.js 192.168.1.45 128 64"
fi
echo
