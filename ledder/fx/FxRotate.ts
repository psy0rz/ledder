...
export default class FxRotate extends Fx {
    ...
    constructor(scheduler: Scheduler, controlGroup: ControlGroup, xStep = -1, yStep = 0, interval = 2, intervalRandomizer = 0, minStep=1) {
        ...
        this.intervalControl = controlGroup.value('Rotate interval', interval, 1, 60, 1, true)
        this.intervalRandomizerControl = controlGroup.value('Rotate interval randomizer', intervalRandomizer, 0, 60, 1, true)
        this.xStepControl = controlGroup.value('Rotate X step', xStep, -5, 5, minStep, true)
        this.yStepControl = controlGroup.value('Rotate Y step', yStep, -5, 5, minStep, true)
