...
export class PlantConfig {
    ...
    setupControls() {
        ...
        const plantTall = tallPlantGroup.value("Count (0=disabled)", 2, 0, 20, 1, true);
        
        const shortPlantGroup = this.controls.group("Short Plants", true, true);
        const plantShort = shortPlantGroup.value("Count (0=disabled)", 2, 0, 20, 1, true);
        
        const bushGroup = this.controls.group("Bushes", true, true);
        const plantBush = bushGroup.value("Count (0=disabled)", 3, 0, 20, 1, true);
        
        const grassGroup = this.controls.group("Grass", true, true);
        const plantGrass = grassGroup.value("Count (0=disabled)", 4, 0, 20, 1, true);

        const fernGroup = this.controls.group("Ferns", true, true);
        const plantFern = fernGroup.value("Count (0=disabled)", 2, 0, 20, 1, true);
