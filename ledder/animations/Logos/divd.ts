...
export default class Hackerhotel extends Animator {
    ...
    async run(box: PixelBox, scheduler: Scheduler, controls: ControlGroup) {
        ...
        const image = await sharp('images/divd1.png')
       ...
       //  await autoTraceFx.run(traces, box)
        ...
        // const trace=new FxTrace(scheduler, controls)
        // trace.run(logo, box)

        if (controls.switch('Color pattern', false ,true).enabled) {
            const fx = new FxColorPattern(scheduler, controls)
            fx.run(logo)
        }
